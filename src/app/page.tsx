...
import { doc, serverTimestamp, setDoc, getDoc, runTransaction, writeBatch, increment, collection, where, query, getDocs } from 'firebase/firestore';
...
export default function AuthPage() {
        ...
        await runTransaction(firestore, async (transaction) => {
          ...
          if (!referrerDoc.exists()) {
            ...
            transaction.update(referrerRef, { 
              ...
              unverifiedBalance: increment(3) // Add 3 unverified tokens
            });
          }
        });
      } else {
  ...
  const handleLogin = async (values: z.infer<typeof loginSchema>) => {
    try {
      ...
      if (user.emailVerified) {
        // The AuthWrapper in layout.tsx will handle the redirect to /home
      } else {
        // Don't sign out. Let the user stay on the page. Resend verification.
        await sendEmailVerification(user);
        toast({
          variant: 'destructive',
  ...
  const handleSocialSignIn = async (provider: GoogleAuthProvider) => {
    ...
    try {
        ...
        // The AuthWrapper in layout.tsx will handle the redirect
    } catch (error: any) {
        let description = 'An unknown error occurred. Please try again.';
